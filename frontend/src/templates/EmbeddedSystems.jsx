...
export default function EmbeddedSystems({ data, portfolioData }) {
  ...
  const bio = personal.bio || '/* No bio provided. Populate personal.bio in the datasheet source to replace this comment block. */';
  ...
  const safeSkills = Array.isArray(skills) && skills.length > 0 ? skills : ['C', 'ADD_SKILLS', 'TO_ARRAY'];
  ...
  const cornerMarks = (color) => (
    <>
      {[
        ...
        <span
          key={i}
          style={{ position: 'absolute', width: '10px', height: '10px', borderStyle: 'solid', borderColor: color, ...pos }}
        />
      ))}
    </>
  );
  ...
  return (
    <div style={{ width: '100%', minHeight: '100vh', background: colors.bg, backgroundImage: `linear-gradient(${colors.line}22 1px, transparent 1px), linear-gradient(90deg, ${colors.line}22 1px, transparent 1px)`, backgroundSize: '32px 32px', color: colors.white, fontFamily: sans, boxSizing: 'border-box', overflowX: 'hidden' }}>
      {/* TOP SPEC BAR */}
      <div style={{ ...wrapper, display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', padding: '1rem 5vw', borderBottom: `1px solid ${colors.line}`, fontFamily: mono, fontSize: '0.75rem', color: colors.muted, gap: '0.5rem' }}>
        <span>REV: 1.0.0</span>
      ...
      <div style={{ ...wrapper, paddingTop: 'clamp(2.5rem, 6vh, 4rem)', paddingBottom: 'clamp(2.5rem, 6vh, 4rem)' }}>
        <div style={{ ...panelStyle, padding: 'clamp(1.75rem, 4vw, 3rem)' }}>
          ...
          <h1 style={{ fontFamily: mono, fontSize: 'clamp(1.8rem, 5vw, 3.4rem)', fontWeight: 700, color: colors.white, margin: 0, lineHeight: 1.1, wordBreak: 'break-word' }}>
            {name}
          </h1>
          <p style={{ fontFamily: mono, fontSize: 'clamp(0.95rem, 1.8vw, 1.2rem)', color: colors.cyan, margin: '0.6rem 0 1.5rem 0', letterSpacing: '0.03em' }}>
            {role}
          </p>
          <p style={{ fontFamily: sans, fontSize: 'clamp(0.9rem, 1.2vw, 1.02rem)', color: colors.muted, lineHeight: 1.75, maxWidth: '70ch', margin: 0 }}>
            {bio}
          </p>
      ...
      <div style={{ ...wrapper, paddingBottom: 'clamp(2.5rem, 6vh, 4rem)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '0.75rem' }}>
          {safeSkills.map((skill, i) => {
            ...
            return (
              <div key={`${skill}-${i}`} onMouseEnter={() => setHoveredSkill(i)} onMouseLeave={() => setHoveredSkill(null)} style={{ ...panelStyle, padding: '0.85rem 1rem', borderColor: isHovered ? colors.cyan : colors.line, background: isHovered ? colors.bgPanelAlt : colors.bgPanel, transition: 'all 0.2s ease', display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                <span style={{ width: '8px', height: '8px', borderRadius: '50%', background: isHovered ? colors.cyan : colors.lineBright, flexShrink: 0, transition: 'background 0.2s ease' }} />
                <span style={{ fontFamily: mono, fontSize: '0.82rem', color: isHovered ? colors.cyan : colors.white, transition: 'color 0.2s ease', wordBreak: 'break-word' }}>
                  {skill}
                </span>
              </div>
            );
      ...
      <div style={{ ...wrapper, paddingBottom: 'clamp(3.5rem, 8vh, 5rem)' }}>
        ...
        <div style={{ ...panelStyle, overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 2fr 1fr', gap: '1rem', padding: '0.85rem 1.25rem', borderBottom: `1px solid ${colors.line}`, background: colors.bgPanelAlt }}>
          ...
          {safeEducation.map((edu, i) => (
            <div key={edu.id || i} style={{ display: 'grid', gridTemplateColumns: '2fr 2fr 1fr', gap: '1rem', padding: '1rem 1.25rem', borderBottom: i === safeEducation.length - 1 ? 'none' : `1px solid ${colors.line}` }}>
              <span style={{ fontFamily: sans, fontSize: '0.9rem', color: colors.white, wordBreak: 'break-word' }}>{edu.institution || 'Institution not set'}</span>
              <span style={{ fontFamily: sans, fontSize: '0.9rem', color: colors.muted, wordBreak: 'break-word' }}>{edu.degree || 'Degree not specified'}</span>
              <span style={{ fontFamily: mono, fontSize: '0.85rem', color: colors.cyan, textAlign: 'right' }}>{edu.score || 'N/A'}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* FOOTER */}
      <div style={{ borderTop: `1px solid ${colors.line}`, padding: '1.5rem 5vw', fontFamily: mono, fontSize: '0.72rem', color: colors.muted, textAlign: 'center' }}>
...
function SectionLabel({ index, title, colors, mono }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
      <span style={{ fontFamily: mono, fontSize: '0.75rem', color: colors.bg, background: colors.cyan, padding: '0.2rem 0.5rem', borderRadius: '2px', fontWeight: 700 }}>{index}</span>
      <span style={{ fontFamily: mono, fontSize: '0.85rem', color: colors.white, letterSpacing: '0.08em' }}>{title}</span>
...
function SpecLink({ label, href, colors, mono }) {
  ...
  return (
    <a href={isActive ? href : undefined} target={isActive && typeof href === 'string' && !href.startsWith('mailto:') ? '_blank' : undefined} rel={isActive ? 'noopener noreferrer' : undefined} style={{ fontFamily: mono, fontSize: '0.78rem', color: isActive ? colors.cyan : colors.muted, textDecoration: 'none', border: `1px solid ${isActive ? colors.cyanDim : colors.line}`, borderRadius: '2px', padding: '0.5rem 1rem', transition: 'all 0.2s ease', cursor: isActive ? 'pointer' : 'default' }} onMouseEnter={(e) => { if (isActive) { e.currentTarget.style.background = colors.bgPanelAlt; e.currentTarget.style.borderColor = colors.cyan; } }} onMouseLeave={(e) => { if (isActive) { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = colors.cyanDim; } }}>
